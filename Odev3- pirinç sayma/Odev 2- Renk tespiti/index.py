...
while True:
    ...
    ust_deger_red = np.array([10, 255, 255])

    # Mavi için aralık
    alt_deger_blue = np.array([75, 100, 100])
    ust_deger_blue = np.array([125, 255, 255])

    #yeşil için aralık
    alt_deger_green = np.array([50, 100, 100])
    ust_deger_green = np.array([100, 255, 255])

    # Kırmızı tonunda pikselleri beyaz, mavi ve yeşil tonunda pikselleri siyah yapan kod
    mask_red = cv2.inRange(hsv, alt_deger_red, ust_deger_red)
    mask_black = cv2.inRange(hsv, alt_deger_blue, ust_deger_blue) | cv2.inRange(hsv, alt_deger_green, ust_deger_green)
